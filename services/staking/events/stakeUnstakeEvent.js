...
const { STAKE_UNSTAKE_EVENT } = require('../constant.js');
...
class StakeUnstakeEvent extends StakeEvent {
 ...
 * @returns {StakeUnstakeEvent}
 */
function unpackStakeUnstakeEvent(u8as) {
  if (u8as[0][0] !== STAKE_UNSTAKE_EVENT[0]) {
  ...
  const event = new StakeUnstakeEvent();
